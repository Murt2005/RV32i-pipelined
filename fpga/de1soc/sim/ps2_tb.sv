// Drives the PS/2 receiver with clean and corrupt frames
...
module ps2_tb;
    ...
    // Short timeout so the watchdog test runs quickly
    ...
    // One bit at about 16.7 kHz, the slow end of the spec
    ...
    // A whole frame, with the parity bit chosen by the caller
    ...
    initial begin
        ...
        // Clean scancode: 0x1C is 'A' in set 2
        send_good(8'h1C);
        ...
        // Break code: F0 then the key
        send_good(8'hF0);
        send_good(8'h1C);
        check("break sequence accepted", got_codes, 3);
        check("  last value",            last_code, 8'h1C);

        // 0x00, whose parity bit is 1, must not look like an idle line
        send_good(8'h00);
        ...
        // The receiver isn't wedged by the bad frame
        send_good(8'h29);
        ...
        // Frame abandoned half way: the watchdog must free it
        send_bit(1'b0);                       // start
        send_bit(1'b1);
